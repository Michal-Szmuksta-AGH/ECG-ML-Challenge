...
class TestECGDataset(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for i in range(5):
            np.savez(
                os.path.join(cls.test_dir, f"test_file_{i}.npz"),
                x=np.random.rand(5000, 1),
                y=np.random.randint(0, 2, size=(5000, 1)),
            )
    ...
    def test_various_num_channels(self):
        ...
        try:
            ...
            for i, channel in enumerate(channels):
                np.savez(
                    os.path.join(test_dir, f"test_file_{i}.npz"),
                    x=np.random.rand(fixed_size, channel),
                    y=np.random.randint(0, 2, size=(fixed_size, 1)),
                )
    ...
    def test_various_lengths(self):
        ...
        try:
            ...
            for i, size in enumerate(sizes):
                np.savez(
                    os.path.join(test_dir, f"test_file_{i}.npz"),
                    x=np.random.rand(size, fixed_channel),
                    y=np.random.randint(0, 2, size=(size, 1)),
                )
            dataset = ECGDataset(test_dir)
            for i, size in enumerate(sizes):
                x, y = dataset[i]
